def buildCmd(id, pairs, singleCell, maxMismatch, method, pairtoolsDedupParams, cpus) {
    ...
    def output = "${id}.dedup.pairs.gz"

    def cmd = "pairtools dedup --output '${output}' ${pairtoolsDedupParams.join(' ')}  --nproc-in ${cpus} --nproc-out ${cpus} '${pairs}'"
    def logMap = [task: "DEDUP_PAIRS", output: output, input: [id: id, pairs: pairs, singleCell: singleCell, maxMismatch: maxMismatch, method: method, pairtoolsDedupParams: pairtoolsDedupParams]]
