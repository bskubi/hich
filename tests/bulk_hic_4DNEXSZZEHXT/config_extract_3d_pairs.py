...
def statistics():
    return {
        "mapq": Category(
            lambda df: (
                pl.concat([df.select(mapq = pl.col.mapq1), df.select(mapq = pl.col.mapq2)])
            ),
            pivot_on = "mapq"
        ),
        "pair_type": Category(lambda df: df.select("pair_type"), pivot_on = "pair_type"),
        ...
        "cis_trans": Category(
            lambda df: (
                ...
                .select(cis_trans = pl.when(pl.col.chrom1 == pl.col.chrom2).then(pl.lit("Cis")).otherwise(pl.lit("Trans")))
            ),
            pivot_on = "cis_trans"
        )
    }
