...
base_command="/usr/bin/time nextflow run contents/main.nf -profile localPC -c tests/nextflow.config -w testwork -resume--apptainer.enabled true"

# Tests: Making bwa aligner index and other input resources
run "Run one HiC 3.0 (DdeI,DpnII) downsampled sample from Akgol 2021 with bwa." "$base_command --sampleFile vignettes/akgol2021/one_rep_bwa.tsv -params-file tests/onerep_bulk.yml"

# Tests: Tiered merge and deduplication
run "Run a tiered HiC 3.0 (DdeI,DpnII) downsampled condition from Akgol 2021 with bwa using resource files created in the last test." "$base_command --sampleFile vignettes/akgol2021/samples_bwa.tsv -params-file tests/tiered_bulk.yml"
