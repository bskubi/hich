...
class ConfigBamParsePairs(ConfigProcess):
    ...
    command_samtools_view: Command = Command(base_command="samtools view", flags=["-b"])
    command_pairtools_parse2: Command = Command(base_command="pairtools parse2", opts={"--chroms-path": "${chromsizes}"})
    command_pairtools_sort: Command = Command(base_command="pairtools sort", opts={"--tmpdir": ".", "--nrpoc-in": "${cpus}", "--nproc-out": "${cpus}", "--output": "${pairs}"})
